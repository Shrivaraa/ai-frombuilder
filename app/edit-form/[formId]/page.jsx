...
import { useRouter, useParams } from "next/navigation"; // Use useParams here
...
function EditForm() {
  ...
  const { formId } = useParams(); // Use useParams to get the formId
  const { user } = useUser();
  ...
  useEffect(() => {
    if (user && formId) GetFormData(); // Ensure formId is defined
  }, [user, formId]); // Added formId to the dependency array
