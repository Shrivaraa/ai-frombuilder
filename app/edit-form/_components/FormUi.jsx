...
function FormUi({
  ...
  const onFormSubmit = async (event) => {
    ...
    try {
      console.log("Form ID for submission:", formId);
      ...
      if (!existingForm || existingForm.length === 0) {
        toast.error("Invalid form. Please try again later.");
        return;
      }

      const result = await db.insert(userResponses).values({
        jsonResponse: JSON.stringify(formData), // Make sure it's stringified!
      ...
      toast.error("Error while submitting form data.");
    }
  };
